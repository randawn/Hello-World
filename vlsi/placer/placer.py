...
class NetList(object):
    ...
    def InputParser(self):
        ...
        gatesk = {}
        padsk = {}
        fh = open('toy')   # no comment for parsing!!!
    ...
    def GetMatC(self, gates):
        ...
        for i in range(len(IDseq)):
            gates[IDseq[i]]['num'] = i
        for (id0, id1) in list(combinations(gates.keys(), 2)):
            try:
                for gate_ID, weight in gates[id1]['gates']:
                    if (id0 == gate_ID):
                        R_lst.append(gates[id0]['num'])
                        R_lst.append(gates[id1]['num'])
                        C_lst.append(gates[id1]['num'])
                        C_lst.append(gates[id0]['num'])
                        V_lst.append(weight)
                        V_lst.append(weight)
            except KeyError as e:
                print e
                continue        # this gate does not has a gate friend
